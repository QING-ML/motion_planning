...
{   
    gl_xl = global_xyz_l(0); //coordinate low and up
    ...
    GLX_SIZE = max_x_id; // max index in different axis
    GLY_SIZE = max_y_id;
    GLZ_SIZE = max_z_id;
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    //Euclidean distance
    double x1=node1->index[0];
    double y1=node1->index[1];
    double z1=node1->index[2];

    double x2=node1->index[0];
    double y2=node1->index[1];
    double z2=node1->index[2];

    //double Euclidean_distance = std::sqrt(pow((x1-x2),2)+pow((y1-y2),2)+pow((z1-z2),2));

    // Manhattan distance
    double Manhattan_distance = abs(x1 - x2) + abs(y1 - y2) + abs(z1 - z2);

    //diagonal distance 3D
    ...
    double diagonal_distance = (std::sqrt(3) - std::sqrt(2))*dmin + (std::sqrt(2) - 1)*dmid + dmax;

//    //Tie Breaker
    double p = 1/(Manhattan_distance);

    //return Euclidean_distance;
    //return Manhattan_distance;
    double Tie_Breaker = diagonal_distance*(1+p);
    return Tie_Breaker;
    //return 0;
}
    ...
    while ( !openSet.empty() ){
        ...
        */
        ...
        if( currentPtr->index == goalIdx ){
            ...
            ROS_WARN("[A*]{sucess}  Time in A*  is %f ms, path cost if %f m", (time_2 - time_1).toSec() * 1000.0, currentPtr->gScore * resolution );  //gscore is index
            return;
        }
        //get the succetion
