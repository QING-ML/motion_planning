...
class Node:
    ...
    def generate_g_value(px, py, vx, vy):
        rand_end = FINISH_LINE[np.random.randint(low=0, high=2, size=1)[0]]
    ...
    def safety_constraints(self, px2, py2, grid):
        ...
        x_way_points = np.linspace(self.px, px2, step + 1, endpoint=True) #interpolation
    ...
    def control(self, ux, uy, grid, success): #return key
